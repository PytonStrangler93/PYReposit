...
print(result)
# Метод позволяющий искать список всех найденных подстрок в нашей строке и
# компанует в список
...
# когда мы пишем шаблон для регулярного выражения нам необходимо
# использовать сырую строку она начинается с r
result = re.search(r'md.k', s1)
# означает что строка после r будет сырой это значит что в ней отключено
# экранирование если мы пишем /r то строка будет выводиться с новой
# строчки если /t то будет табуляция r это отключает.
print(result)  # Точка здесь заменяет символ и игнорт его кроме символа новой строки
# Выводит любую цифру по очереди можно дублировать (при дубле выводит
# только цифры подряд)
result = re.search(r'\d', s1)
print(result)
result = re.search(r'\D', s1)  # Выводит любой символ первый который встретит
print(result)
# Выводит любой пробельный символ (Табуляция и т.п.)
result = re.search(r'\s', s1)
print(result)
...
print(result)
# Выводит НЕ букву НЕ цифру НЕ нижнее подчеркивание
result = re.search(r'\W', s1)
print(result)
# Выводит начало или конец какого-либо слова FD ЭТО БУКВЫ НАЧАЛА СЛОВА
result = re.search(r'\bFD', s1)
print(result)
# Не укажет границы слова потому-что перед указанными символами есть
# другеи символы
result = re.search(r'\BFD', s1)
print(result)
# Выводит 0 или более вхождения каких-то символов после нашей первой
# найденной цифры если я пишу \d+ то 1 или более вхождений
result = re.search(r'\d*', s1)
print(result)
# Выводит результат если встретится одна из перечисленных букв или
# диапазон [4-8]
result = re.search(r'[mdfk]', s1)
print(result)
...
print(result)
# Ко всему вышеперечисленному мы можем применить квантификаторы - кол-во
# повторений данного шаблона
# Выводит \d 4 раза или диапазон {1,4} (выведет 3 числа) или не менне
# скольки-то повторений {5,} (не менее 5 подряд идущих цифр) {,5} (не
# более 5)
result = re.search(r'\d{4}', s1)
print(result)
# ПРИМЕР!!
s2 = "Привет! Как дела! А у меня нормально."
# Ищет слова начинающиеся на буквы w+ значит что он будет искать все слово
# а не только расположение символов
result = re.findall(r'[бвгджзиклмнпрстфхчшщБВГДЖЗИКЛМНПРСТФХЧШЩ]\w+', s2)
print(result)
